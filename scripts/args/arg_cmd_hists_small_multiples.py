...
def extract_context(cleanarg, rawlist):
    ...
    for arg in rawlist:
        ...
        for (command, query) in cursor.fetchall():
            ...
            if command.find("REX") == 0 or command.find("REGEX") == 0:
                c.grp_field_used_in_other_transformation += 1
                set = True

            if command.find("MULTIKV") == 0 or command.find("CONVERT") == 0 or command.find("BUCKET") == 0 or command.find("REX") or command.find("REPLACE"):
                c.grp_field_used_in_other_transformation += 1
                set = True
...
def append_plot(context, i):
    ...
    counts = [c+.0001 for c in counts]
    print context.arg, counts
...
def main():
    ...
    #plt.figlegend(bars, context.get_labels(), "upper right")
    plt.show()
